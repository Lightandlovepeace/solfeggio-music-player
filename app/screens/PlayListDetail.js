...
const PlayListDetail = (props) => {
    ...
    await selectAudio(audio, context, 
      {activePlayList: playList, isPlayListRunning: true});
  };
  ...
  const removeAudio = async ()=> {
    ...
    if(result !== null){
      ...
      context.updateState(context, {playList: updatedPlayLists, isPlayListRunning,
      ...
      <View style={styles.container}>
        <View style ={{
          ...
          <TouchableOpacity onPress={removePlaylist}>
            <Text style={[styles.title,{color:'red'}]}>Delete Playlist</Text>
          </TouchableOpacity>
        </View>
        {audios.length ? (
          <FlatList
